  ...
  return (product?
    <div className='flex justify-between w-full  mx-auto max-w-[77rem] font-lato border-b-solid border-b-[1px] border-b-[#BBBBBB] py-[2.53125rem]' >
        <div className='flex gap-[13.15px] md:gap-[3.25rem]'>
            <div>
            <img className='w-[62.85px] h-[full]' src={product.image} alt="" />
            </div>
            <div>
                <p className='font-[600] text-[1rem] md:text-[1.5rem]'>{product.Name}</p>
                <p className='font-[400] font-[1rem] text-[12px] md:text-[1rem]'>Written by: {product.author}</p>
                <p className='text-[9.96px] font-[500] md:text-[initial]'>Category: {product.category}</p>
            </div>
        </div>
    ...
    </div>:loading? <div className='flex justify-center items-center'> <SyncLoader /> </div> :<div> Couldnt Load Image </div>
  )
}
