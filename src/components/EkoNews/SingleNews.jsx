...
const SingleNews = ({img, title, text, path}) => {
...
const fulltextStyle = ['h-200px md:h-[100px] transition ease-in-out delay-150 duration-300']
  ...
  return (
    <div className='flex justify-between gap-[4.125rem] flex-col md:flex-row' >
...
<div className='font-lato font-[400] text-[16px] md:text-[1.25rem] text-[#232E52]'>    
    <div className={inc?fulltextStyle:textStyle}>{text}</div>
    <span className='text-[#5A0C91] text-[18.75px]' onClick={() => setinc(!inc)}>
    {!inc?"...Read More":"Less"}
    </span>
</div>
        </div>
